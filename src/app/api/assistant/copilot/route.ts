...
import { handleUserMessageCopilot } from "@/services/copilot.service";

const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
const assistantId = process.env.ASSISTANT_COPILOT_ID!;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { stream } = await handleUserMessageCopilot({
      assistantId,
      threadId: thread.id,
      message,
      ip,

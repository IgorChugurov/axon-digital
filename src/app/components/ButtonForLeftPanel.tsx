...
const ButtonForLeftPanel = ({
  hideWhenExpand,
}: {
  hideWhenExpand?: boolean;
}) => {
  ...
  useEffect(() => {
    ...
    if (leftPanelState) {
      try {
        setIsLeftPanelOpen(JSON.parse(leftPanelState));
      } catch {}
    } else {
    ...
    const handleToggleLeftPanel = (event: CustomEvent) => {
      ...
      localStorage.setItem("leftPanel", JSON.stringify(detail));
    };
  ...
  if (hideWhenExpand && isLeftPanelOpen) {
    return <></>;
  }
  return (
    <button
      className="p-1 rounded-md hover:bg-gray-200 transition-colors cursor-pointer"
      onClick={() => sendMessage("toggleLeftPanel", !isLeftPanelOpen)}
    >
      <svg
        width="24"
        height="24"
...
export default ButtonForLeftPanel;
